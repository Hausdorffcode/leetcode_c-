...
//和上面的代码是一样的，只是把Dijkstra用结构体包起来，注意要声明在mian外
#include<bits/stdc++.h>
using namespace std;
...
int main() {
    ...
    for (int i = 1; i < nodes.size(); i++) {
        if (nodes[i].x == nodes[i-1].x) {
            dij.AddEdge(nodes[i].id, nodes[i-1].id, 0);
            dij.AddEdge(nodes[i-1].id, nodes[i].id, 0);
        }
        else {
            dij.AddEdge(nodes[i].id, nodes[i-1].id, nodes[i].x - nodes[i-1].x);
            dij.AddEdge(nodes[i-1].id, nodes[i].id, nodes[i].x - nodes[i-1].x);
        }
    }
    sort(nodes.begin(), nodes.end(), cmp2);
    for (int i = 1; i < nodes.size(); i++) {
        if (nodes[i].y == nodes[i-1].y) {
            ...
            dij.AddEdge(nodes[i].id, nodes[i-1].id, nodes[i].y - nodes[i-1].y);
            dij.AddEdge(nodes[i-1].id, nodes[i].id, nodes[i].y - nodes[i-1].y);
        }
    }
